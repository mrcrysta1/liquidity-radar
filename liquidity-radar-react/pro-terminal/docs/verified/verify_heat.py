...
with sync_playwright() as p:
    ...
    pg.click('nav.rail button[title="Heatmap"]'); pg.wait_for_timeout(3500)
    pg.screenshot(path='/tmp/h1_heatmap_full.png')
    box=pg.query_selector('.heat-wrap').bounding_box(); pg.mouse.move(box['x']+box['width']*0.7, box['y']+box['height']*0.35); pg.wait_for_timeout(300)
    pg.screenshot(path='/tmp/h2_heatmap_hover.png')
    pg.click('.seg >> text=Model 3'); pg.click('.cmaps .cmap >> nth=1'); pg.wait_for_timeout(800); pg.screenshot(path='/tmp/h3_model3_magma.png')
    pg.click('nav.rail button[title="Terminal"]'); pg.wait_for_timeout(500); pg.click('.drawer .tabs >> text=Liq. heatmap'); pg.wait_for_timeout(2500); pg.screenshot(path='/tmp/h4_drawer.png')
