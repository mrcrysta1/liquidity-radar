export interface NewsItem { id: string; title: string; link: string; source: string; ts: number; summary?: string; category: 'crypto' | 'markets' | 'macro' | 'regulatory' | 'exchange' }
export interface NewsSource { id: string; name: string; url: string; category: NewsItem['category'] }
...
export function parseFeed(xml: string, source: NewsSource): NewsItem[] {
  const doc = new DOMParser().parseFromString(xml, 'text/xml'); const out: NewsItem[] = [];
  const items = [...doc.querySelectorAll('item')]; const entries = items.length ? items : [...doc.querySelectorAll('entry')];
  for (const it of entries.slice(0, 40)) {
    const title = strip(txt(it, 'title')); if (!title) continue;
    const link = txt(it, 'link') || it.querySelector('link')?.getAttribute('href') || '';
    const date = txt(it, 'pubDate') || txt(it, 'published') || txt(it, 'updated') || it.getElementsByTagName('dc:date')[0]?.textContent || '';
    const ts = Date.parse(date) || Date.now(); const summary = strip(txt(it, 'description') || txt(it, 'summary') || txt(it, 'content')).slice(0, 240);
...
export function dedupe(items: NewsItem[]): NewsItem[] {
  ...
  for (const it of [...items].sort((a, b) => a.ts - b.ts)) { const k = it.title.toLowerCase().replace(/[^a-z0-9 ]/g, '').split(' ').filter(Boolean).slice(0, 6).join(' '); const p = seen.get(k); if (p && Math.abs(p.ts - it.ts) < 6 * 3600e3) continue; seen.set(k, it); }
...
export function importance(it: NewsItem): number {
  const t = it.title.toLowerCase(); let s = 0;
