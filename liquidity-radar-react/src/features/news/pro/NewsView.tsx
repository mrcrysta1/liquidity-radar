// Ported from pro-terminal/src/components/NewsView.tsx (Pro Terminal news view)
// into the main app's News tab.
import { useEffect, useMemo, useState } from 'react'
import { useProNews } from './useNews'
import { NEWS_SOURCES, importance, type NewsItem } from './rss'
import { groupByDay, actualTone, type CalendarEvent, type Impact } from './forexFactory'

const ago = (ts: number) => {
  const m = Math.max(0, Math.round((Date.now() - ts) / 60000))
  return m < 1 ? 'now' : m < 60 ? `${m}m` : m < 1440 ? `${Math.floor(m / 60)}h` : `${Math.floor(m / 1440)}d`
}
const CATS: { id: NewsItem['category'] | 'all'; l: string }[] = [
  { id: 'all', l: 'All' },
  { id: 'crypto', l: 'Crypto' },
  { id: 'markets', l: 'Markets' },
  { id: 'macro', l: 'Macro' },
  { id: 'regulatory', l: 'Regulatory' },
]
...
export function NewsView({ compact = false }: { compact?: boolean }) {
  const { items, sourceStatus, lastNews, refreshNews, calendar, calendarStatus, calendarError, lastCalendar, refreshCalendar, calendarWeek } = useProNews()
  const [tab, setTab] = useState<'news' | 'calendar'>('news')
  const [q, setQ] = useState('')
  const [cat, setCat] = useState<typeof CATS[number]['id']>('all')
  const [src, setSrc] = useState('all')
  const [imp, setImp] = useState<Set<Impact>>(new Set(['High', 'Medium', 'Low', 'Holiday']))
  const [ccy, setCcy] = useState<Set<string>>(new Set(CCY))
  const [now, setNow] = useState(() => Date.now())
  useEffect(() => {
    refreshNews()
    refreshCalendar()
    const a = setInterval(refreshNews, 60_000)
    const b = setInterval(() => refreshCalendar(), 5 * 60_000)
  ...
  const days = useMemo(
    () => groupByDay(calendar.filter((e) => imp.has(e.impact) && (ccy.has(e.currency) || !CCY.includes(e.currency)))),
    [calendar, imp, ccy],
  )
    ...
    <section className={`news ${compact ? 'compact' : ''}`} aria-label="News and calendar">
        ...
        {tab === 'news' ? (
          <>
            ...
            <span className="stat-chip">
              <i className={okCount ? 'dot ok' : 'dot bad'} />
              {okCount}/{NEWS_SOURCES.length} feeds · {lastNews ? `updated ${ago(lastNews)} ago` : 'loading…'}
            </span>
          ...
          <>
            <div className="seg">
              <button className={calendarWeek === 'this' ? 'on' : ''} onClick={() => refreshCalendar('this')}>
                This week
              </button>
              <button className={calendarWeek === 'next' ? 'on' : ''} onClick={() => refreshCalendar('next')}>
                Next week
              </button>
            </div>
            <div className="seg impact-filter">
              {IMPACTS.map((i) => (
                <button
                  key={i}
                  className={imp.has(i) ? 'on' : ''}
                  onClick={() => toggle(imp, i, setImp)}
                  title={`${i} impact`}
                >
                  <span className={`ff-impact ${i.toLowerCase()}`} />
                  {i}
                </button>
              ))}
            </div>
            <div className="seg ccy-filter">
              {CCY.map((c) => (
                <button key={c} className={ccy.has(c) ? 'on' : ''} onClick={() => toggle(ccy, c, setCcy)}>
                  {c}
                </button>
              ))}
            </div>
            <span className="spacer" />
            <span className="stat-chip">
              <i className={`dot ${calendarStatus === 'ok' ? 'ok' : calendarStatus === 'error' ? 'bad' : ''}`} />
              Forex Factory ·{' '}
              {calendarStatus === 'ok' && lastCalendar
                ? `updated ${ago(lastCalendar)} ago`
      ...
      {tab === 'news' ? (
        <div className="news-list">
          {list.length === 0 && (
            <div className="empty">
              {lastNews
                ? 'No headlines match.'
                : 'Loading feeds… (RSS is fetched through the local proxy; run `npm run dev` or `node server/proxy.mjs`).'}
            </div>
          )}
          {list.map((it) => {
            const s = importance(it)
            return (
              <a key={it.id} className={`news-row imp${s}`} href={it.link} target="_blank" rel="noreferrer noopener">
                ...
                <span className="news-body">
                  ...
                  {!compact && it.summary && <span className="news-sum">{it.summary}</span>}
                </span>
        ...
        <div className="ff">
          {calendarStatus === 'error' && (
            <div className="empty">
              Calendar unavailable: {calendarError}. The feed is fetched via the proxy (allow-listed host nfs.faireconomy.media).
            </div>
          )}
          <table className="ff-table">
            <thead>
              <tr>
                <th className="ff-date">Date</th>
                <th className="ff-time">Time</th>
                <th className="ff-ccy">Currency</th>
                <th className="ff-imp">Impact</th>
            ...
            <tbody>
              {days.map((g) => {
                ...
                return [
                  ...
                  ...g.events.map((e) => {
                    const t = e.allDay || e.impact === 'Holiday' ? 'All Day' : new Date(e.ts).toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit' })
          ...
          <div className="ff-legend">
            ...
            <span className="ff-impact low" /> Low <span className="ff-impact holiday" /> Holiday · times in your local
            timezone · actual shown green when better than forecast for the currency, red when worse
          </div>
        </div>
      )}
    </section>
